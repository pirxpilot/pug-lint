  ...
  lint(file, errors) {
    file.iterateTokensByType('start-attributes', token => {
      ...
      file.iterateTokensByFilter(
        token => token.type === 'attribute' && token._index > attributesStartIndex && token._index < attributesEndIndex,
        token => {
      ...
      file.iterateTokensByFilter(
        token => token.type === 'id' && token._index >= boundaryStartIndex && token._index <= boundaryEndIndex,
        token => {
