...
Rule.prototype = {
  ...
  configure(options) {
    ...
    assert(
      typeof options.separator === 'string' && /^([ ,]+)$/.test(options.separator),
    ...
    assert(
      ...
      `${this.name} option.multiLineSeparator requires string value containing only commas, spaces or tabs`
    );
  ...
  lint(file, errors) {
    ...
    file.iterateTokensByType('start-attributes', token => {
      ...
      if (endIndex > startIndex + 2) {
        file.iterateTokensByFilter(
          token => token.type === 'attribute' && token._index > startIndex && token._index < endIndex,
          token => {
            const value = typeof token.val === 'boolean' ? '' : token.val.trim();

            patterns.push(
              getEscapedPattern(token.name) + (value.length > 0 ? '\\s*\\!*=\\s*' : '') + getEscapedPattern(value)
            );
    ...
    function getParsedSource(current, start, end, patterns) {
      ...
      source = source.replace(regexReplace, val => Array(val.length + 1).join('#'));
