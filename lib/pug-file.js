...
PugFile.prototype = {
  addErrorForAllStaticAttributeValues(name, errors, message) {
    this.iterateTokensByFilter(
      token => token.type === 'attribute' && token.name.toLowerCase() === name.toLowerCase(),
      token => {
        const options = {
          terse: true,
          format: 'html',
  ...
  addErrorForIncorrectSpacesAroundType(options, errors) {
    options.filter = token => token.type === options.type;
  ...
  addErrorForIncorrectTokenTypeOrder(typeA, typeB, boundaryTypes, errors, message) {
    ...
    this.iterateTokensByType(typeB, token => {
      ...
      const start = this.getPreviousTokenByType(token, boundaryTypes) || current;
  ...
  findStartLocation(value, start, end) {
    const lines = this.getLines(start.line, end?.line);
  ...
  getNextTokenByType(current, type, direction) {
    ...
    return this.getNextTokenByFilter(current, current => type.indexOf(current.type) !== -1, direction);
  },
  ...
  iterateTokensByFilter(filter, cb) {
    this.getTokensByFilter(filter).forEach(token => {
      cb(token);
    });
  },
