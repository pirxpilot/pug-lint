...
function createTest(linter, fixturesPath) {
  describe('disallowSpaceAfterCodeOperator', () => {
    describe('true', () => {
      before(() => {
        linter.configure({ disallowSpaceAfterCodeOperator: true });
      });

      it('should report space after unbuffered operator', () => {
        assert.equal(linter.checkString('- This is code').length, 1);
      });

      it('should report space after buffered operator', () => {
      ...
      it('should not report missing space after unbuffered operator', () => {
      ...
      it('should not report missing space after escaped buffered operator', () => {
      ...
      it('should report multiple errors found in file', () => {
        const result = linter.checkFile(`${fixturesPath}disallow-space-after-code-operator.pug`);
    ...
    describe('Array', () => {
      ...
      it('should not report space after unescaped buffered operator', () => {
        assert.equal(linter.checkString("p!= 'This code is not <escaped>'").length, 0);
      });
    });
  });
