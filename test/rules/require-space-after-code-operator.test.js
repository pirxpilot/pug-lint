...
function createTest(linter, fixturesPath) {
  describe('requireSpaceAfterCodeOperator', () => {
    describe('true', () => {
      before(() => {
        linter.configure({ requireSpaceAfterCodeOperator: true });
      });

      it('should report missing space after unbuffered operator', () => {
        assert.equal(linter.checkString('-This is code').length, 1);
      });

      it('should report missing space after buffered operator', () => {
      ...
      it('should report missing space after escaped buffered operator', () => {
      ...
      it('should not report space after unbuffered operator', () => {
        assert.equal(linter.checkString('- This is code').length, 0);
      });

      it('should not report space after buffered operator', () => {
      ...
      it('should report multiple errors found in file', () => {
        const result = linter.checkFile(`${fixturesPath}require-space-after-code-operator.pug`);
    ...
    describe('Array', () => {
      ...
      it('should not report missing space after buffered operator', () => {
        assert.equal(linter.checkString("p='This code is <escaped>'").length, 0);
      });

      it('should not report missing space after unescaped buffered operator', () => {
        assert.equal(linter.checkString("p!='This code is not <escaped>'").length, 0);
      });
    });
  });
